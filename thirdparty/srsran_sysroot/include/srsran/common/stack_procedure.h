  ...
  void operator()(ArgsRef&&... args) {
    for (auto& f : func_list) {
      if (f.active) {
        ...
        if (not f.call_always) { f.active = false; }
      }
    }
  }
...
private:
  uint32_t get_new_callback() {
    ...
    if (i == func_list.size()) { func_list.emplace_back(); }
...
// Implementation of the Procedure Manager functionality, including launching, trigger events,
// clearing
...
public:
  ...
  explicit proc_t(Args&&... args) : proc_ptr(new T(std::forward<Args>(args)...)) {
    static_assert(
        std::is_same<result_type, decltype(proc_detail::get_result_type(std::declval<T>()))>::value,
  ...
  //! method to handle external events. "T" must have the method "T::react(const Event&)" for the
  ...
  proc_future_type get_future() {
    if (is_idle()) { return proc_future_type{}; }
    if (future_result == nullptr) { future_result = std::make_shared<proc_result_type>(); }
  ...
  //! launch a procedure, returning true if successful or running and false if it error or it failed
  //! to launch
  template <class... Args>
...
public:
  ...
  callback_id_t on_every_trigger(proc_t<Proc, ResultType>& p) {
    return callbacks.on_every_call(
        [&p](EventType&& ev) { p.trigger(std::forward<EventType>(ev)); });
  }
...
class func_proc_t {
public:
  explicit func_proc_t(std::function<proc_outcome_t()> step_func_)
...
public:
  ...
  bool add_task(std::function<proc_outcome_t()> step_func) {
    ...
    if (not proc.launch()) { return false; }
