...
int open_socket(net_utils::addr_family ip, net_utils::socket_type socket_type,
...
bool connect_to(int fd, const char* dest_addr_str, int dest_port,
                sockaddr_in* dest_sockaddr = nullptr);

}  // namespace net_utils

/**
 * Description: Net socket class with convenience methods for connecting, binding, and opening
 * socket
 */
class unique_socket {
public:
  ...
  bool open_socket(net_utils::addr_family ip, net_utils::socket_type socket_type,
                   net_utils::protocol_type protocol);
...
bool sctp_init_socket(unique_socket* socket, net_utils::socket_type socktype,
                      const char* bind_addr_str, int bind_port);
...
/**
 * Description - Instantiates a thread that will block waiting for IO from multiple sockets, via a
 * select The user can register their own (socket fd, data handler) in this class via the
 *               add_socket_handler(fd, task) API or its other variants
 */
class socket_manager final
    : public thread
    , public socket_manager_itf {
...
private:
  ...
  std::map<int, recv_callback_t>::iterator remove_socket_unprotected(int fd, fd_set* total_fd_set,
                                                                     int* max_fd);
...
using sctp_recv_callback_t = srsran::move_callback<void(
    srsran::unique_byte_buffer_t, const sockaddr_in&, const sctp_sndrcvinfo&, int)>;

/// Function signature for SDU byte buffers received from any sockaddr_in-based socket
using recvfrom_callback_t =
    srsran::move_callback<void(srsran::unique_byte_buffer_t, const sockaddr_in&)>;

/**
 * Helper function that creates a callback that is called when a SCTP socket has data, and does the
 * following tasks:
 * 1. receive SDU byte buffer from SCTP socket and associated metadata - sockaddr_in,
 * sctp_sndrcvinfo, flags
 * 2. dispatches the received SDU+metadata+rx_callback into the "queue"
 * 3. potentially on a separate thread, the SDU+metadata+callback are popped from the queue, and
 * callback is called with the SDU+metadata as arguments
 ...
 * @param rx_callback callback that is run when a new SDU arrives, from the thread that calls
 * queue.pop()
 * @return callback void(int) that can be registered in socket_manager
 */
socket_manager_itf::recv_callback_t make_sctp_sdu_handler(srslog::basic_logger& logger,
                                                          srsran::task_queue_handle& queue,
                                                          sctp_recv_callback_t rx_callback);

/**
 * Similar to make_sctp_sdu_handler, but for any sockaddr_in-based socket type
 */
socket_manager_itf::recv_callback_t make_sdu_handler(srslog::basic_logger& logger,
                                                     srsran::task_queue_handle& queue,
                                                     recvfrom_callback_t rx_callback);
