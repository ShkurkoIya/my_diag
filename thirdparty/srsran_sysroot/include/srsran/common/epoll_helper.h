...
public:
  epoll_timer_handler(int fd_, epoll_timer_callback callback_)
      : timer_fd(fd_), callback(callback_) {};
...
public:
  ...
  int handle_event(int fd, epoll_event e, int epoll_fd) {
    ...
    switch (info.ssi_signo) {
      ...
      case SIGINT:
      case SIGHUP:
      case SIGQUIT: running = false; break;
      default: fprintf(stderr, "got signal %d\n", info.ssi_signo); break;
    }
...
inline int add_signalfd() {
  ...
  sigset_t sw;
  sigemptyset(&sw);
  for (auto& sig : sigs) { sigaddset(&sw, sig); }
