...
private:
  ...
  static void merge_passport(CellPassport& dst, const CellPassport& src) {
    ...
    if (src_plmn) {
      // Soft (same-EARFCN fan-out) never overwrites a hard PLMN.
      if (src.plmn_soft) {
        if (!dst_plmn) {
          dst.mcc = src.mcc;
          dst.mnc = src.mnc;
        ...
        dst.mnc = src.mnc;
        dst.plmn_soft = false;
      } else if (dst.plmn_soft) {
        // Hard source confirms / replaces inferred PLMN on this key.
        dst.mcc = src.mcc;
        dst.mnc = src.mnc;
        dst.plmn_soft = false;
      }
    ...
    if (src.mnc_digits == 2 || src.mnc_digits == 3) dst.mnc_digits = src.mnc_digits;
    if (src.rac != 0) dst.rac = src.rac;
    ...
    if (src_full || (!dst_full && (src_eci || src_plmn))) {
      ...
      dst.cell_reserved_for_operator = src.cell_reserved_for_operator;
  ...
  /// Marked plmn_soft — heuristic only; not a FULL identity bind.
  ...
  void upsert_lte_meas_neighbors(LocalCellKey serving, RatType rat,
    ...
    for (const auto& n : neighbors) {
      ...
      if (n.has_rsrp) {
        ...
        if (n.has_rsrp_filt) {
          lp.rsrp_filt = n.rsrp_filt;
          lp.has_rsrp_filt = true;
        }
        if (n.has_rsrq_filt) {
          lp.rsrq_filt = n.rsrq_filt;
        ...
        if (n.has_sinr_per_rx) {
          lp.sinr_rx0 = n.sinr_rx0;
          lp.sinr_rx1 = n.sinr_rx1;
    ...
    std::visit(
        [&](const auto& incoming, auto& target) {
          ...
          if constexpr (!std::is_same_v<S, D>) {
            ...
            if (incoming.has_rsrp_filt && Utils::valid_lte_rsrp(incoming.rsrp_filt)) {
              target.rsrp_filt = incoming.rsrp_filt;
              target.has_rsrp_filt = true;
            }
            if (incoming.has_rsrq_filt && incoming.rsrq_filt >= -30.0f &&
                incoming.rsrq_filt <= -1.0f) {
              target.rsrq_filt = incoming.rsrq_filt;
              target.has_rsrq_filt = true;
            }
            ...
            if (incoming.has_sinr_per_rx) {
              target.sinr_rx0 = incoming.sinr_rx0;
              target.sinr_rx1 = incoming.sinr_rx1;
              target.has_sinr_per_rx = true;
            }
  ...
  void merge_radio_fields(T& target, const T& src) {
    ...
    // PSC 0 is valid on WCDMA — set whenever DL UARFCN is present.
    if constexpr (requires {
                    target.psc;
                    target.dl_uarfcn;
                  }) {
      if (src.dl_uarfcn) target.psc = src.psc;
      else if (src.psc) target.psc = src.psc;
    } else if constexpr (requires { target.psc; }) {
      if (src.psc) target.psc = src.psc;
    }
    if constexpr (requires { target.ura_id; }) {
      if (src.ura_id) target.ura_id = src.ura_id;
    }
    if constexpr (requires { target.access; }) {
      if (src.access) target.access = src.access;
    }
    if constexpr (requires { target.flags; }) {
      if (src.flags) target.flags = src.flags;
    }
    if constexpr (requires { target.last_rrc_channel; }) {
      if (src.last_rrc_channel != 0xFF) {
        target.last_rrc_channel = src.last_rrc_channel;
        target.last_rrc_len = src.last_rrc_len;
      }
    }
    ...
    if constexpr (requires { target.cell_resel_prio; }) {
      if (src.cell_resel_prio) target.cell_resel_prio = src.cell_resel_prio;
    }
    if constexpr (requires { target.thresh_x_high; }) {
      if (src.thresh_x_high) target.thresh_x_high = src.thresh_x_high;
    }
    if constexpr (requires { target.thresh_x_low; }) {
      if (src.thresh_x_low) target.thresh_x_low = src.thresh_x_low;
    }
    if constexpr (requires { target.timing_advance; }) {
      if (src.timing_advance) target.timing_advance = src.timing_advance;
    }
    if constexpr (requires { target.nas_idle; }) {
      if (src.nas_idle >= 0) target.nas_idle = src.nas_idle;
    }
    if constexpr (requires { target.allowed_access; }) {
      if (src.allowed_access >= 0) target.allowed_access = src.allowed_access;
    }
    if constexpr (requires { target.emm_state; }) {
      if (src.emm_state >= 0) {
        target.emm_state = src.emm_state;
        target.emm_substate = src.emm_substate;
      }
    }
    if constexpr (requires { target.emm_mcc; }) {
      if (src.emm_mcc) {
        target.emm_mcc = src.emm_mcc;
        target.emm_mnc = src.emm_mnc;
        target.emm_mnc_digits = src.emm_mnc_digits;
      }
    }
    if constexpr (requires { target.mme_present; }) {
      if (src.mme_present) {
        target.mme_present = true;
        target.mme_group_id = src.mme_group_id;
        target.mme_code = src.mme_code;
      }
    }
    if constexpr (requires { target.rrc_state; }) {
      if (src.rrc_state >= 0) target.rrc_state = src.rrc_state;
    }
    if constexpr (requires { target.ul_nrarfcn; }) {
      if (src.ul_nrarfcn) target.ul_nrarfcn = src.ul_nrarfcn;
    }
    if constexpr (requires { target.band; }) {
      if (src.band) target.band = src.band;
    }
    if constexpr (requires { target.scs_khz; }) {
      if (src.scs_khz) target.scs_khz = src.scs_khz;
    }
    if constexpr (requires { target.sfn; }) {
      if (src.sfn) target.sfn = src.sfn;
    }
    if constexpr (requires { target.has_sfn_sf; }) {
      if (src.has_sfn_sf) {
        target.sfn = src.sfn;
        target.subframe = src.subframe;
        target.has_sfn_sf = true;
      }
    }
    if constexpr (requires { target.valid_rx; }) {
      if (src.valid_rx) target.valid_rx = src.valid_rx;
    }
    if constexpr (requires {
                    target.serving_cell_index;
                    target.has_sfn_sf;
                    target.is_restricted;
                  }) {
      if (src.has_sfn_sf) {
        target.serving_cell_index = src.serving_cell_index;
        target.is_restricted = src.is_restricted;
      }
    }
