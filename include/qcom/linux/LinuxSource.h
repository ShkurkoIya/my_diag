...
public:
  ...
  [[nodiscard]] bool start() override {
    ...
    if (m_cfg.init_masks) {
      ...
      if (!m_init_ok) {
        ...
        m_wanted_lte_pack.store(static_cast<uint8_t>(LteDiagPack::Search),
                                std::memory_order_relaxed);
        m_applied_lte_pack.store(static_cast<uint8_t>(LteDiagPack::Search),
                                 std::memory_order_relaxed);
      }
    } else {
      m_init_ok = true;
  ...
  /// Mid-session LTE SET_MASK (worker applies; never DISABLE_ALL). No-op on WcdmaOnly.
  void request_lte_diag_pack(LteDiagPack pack) {
    if (m_cfg.mask_profile == DiagMaskProfile::WcdmaOnly) return;
    m_wanted_lte_pack.store(static_cast<uint8_t>(pack), std::memory_order_relaxed);
  }

  [[nodiscard]] LteDiagPack wanted_lte_diag_pack() const noexcept {
    return static_cast<LteDiagPack>(m_wanted_lte_pack.load(std::memory_order_relaxed));
  }

  /// Last pack we believe the modem has; Search if not yet applied.
  [[nodiscard]] LteDiagPack applied_lte_diag_pack() const noexcept {
    const uint8_t v = m_applied_lte_pack.load(std::memory_order_relaxed);
    if (v == kLtePackUnknown) return LteDiagPack::Search;
    return static_cast<LteDiagPack>(v);
  }

private:
  static constexpr uint8_t kLtePackUnknown = 0xFF;
  ...
  std::atomic<uint8_t> m_wanted_lte_pack{static_cast<uint8_t>(LteDiagPack::Search)};
  std::atomic<uint8_t> m_applied_lte_pack{kLtePackUnknown};
  std::thread m_worker;
  ...
  bool init_diag_session() {
    ...
    bool ok = session.init_modem(m_cfg.mask_profile);
    if (!ok) {
      std::this_thread::sleep_for(std::chrono::milliseconds(50));
      drain_rx();
      ok = session.init_modem(m_cfg.mask_profile);
    }
    drain_rx();
    return ok;
  }

  void maybe_apply_lte_pack() {
    if (m_cfg.mask_profile == DiagMaskProfile::WcdmaOnly) return;
    if (!m_cfg.init_masks) return;
    if (m_fd < 0) return;
    const uint8_t want = m_wanted_lte_pack.load(std::memory_order_relaxed);
    const uint8_t have = m_applied_lte_pack.load(std::memory_order_relaxed);
    if (want == have) return;
    DiagSession session(
        [this](std::span<const uint8_t> frame) { return write_all(frame.data(), frame.size()); });
    if (session.apply_lte_pack(static_cast<LteDiagPack>(want))) {
      m_applied_lte_pack.store(want, std::memory_order_relaxed);
    }
  }
  ...
  [[nodiscard]] bool revive_serial_on_worker() {
    ...
    for (int attempt = 0; attempt < 20; ++attempt) {
      ...
      // Skip full init_modem — DiagSession during revive can re-HUP the link.
      // Re-apply the wanted LTE pack on the next idle epoll tick.
      m_applied_lte_pack.store(kLtePackUnknown, std::memory_order_relaxed);
